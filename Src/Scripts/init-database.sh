...
/opt/mssql-tools/bin/sqlcmd -S sqlserver -U sa -P SECRET_SA_PASSWORD -d master -i /tmp/tech-challenge-mer-1.sql
exit_code=$?
if [ $exit_code -eq 0 ]; then
    echo "Executando part 2"
    sleep 5
    /opt/mssql-tools/bin/sqlcmd -S sqlserver -U sa -P SECRET_SA_PASSWORD -d tech-challenge-grupo-71 -i /tmp/tech-challenge-mer-2.sql
