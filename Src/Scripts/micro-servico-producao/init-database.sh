...
if [ $exit_code -eq 0 ]; then
    ...
    sleep 5
    /opt/mssql-tools/bin/sqlcmd -S {{ secrets.TEST_SERVER_NAME }} -U {{ secrets.SA_USER }} -P {{ secrets.SA_PASSWORD }} -d tech-challenge-micro-servico-producao-grupo-71 -i /tmp/tech-challenge-mer-2.sql
